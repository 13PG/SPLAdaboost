...
def AMSPL(numIt,classLabels,datToClass,aggClassEst,v,old_lam_1,numIt_threshVal=200,learn_rate=0.7):
	"""
	自适应混合权重的自步学习方法
		numIt 				- 当前自步学习的轮数
		classLabels 		- 样本标签值
	...
	for i in range(rows):               # 按照行来遍历
		for j in range(cols):           # 对第i行进行遍历
			if L[i, j]<=lam_2:
				...
				v[i, j]=pow((lam_1-L[i, j])/(lam_1-lam_2),1/(t-1))
	# print('{}--{}--{}\n{}'.format(lam_1,lam_2,t,v))
	# print(v)
	return v,lam_1
...
def buildStump(dataArr,classLabels,D,V,aggClassEst,C=0.53):
	"""
	找到数据集上最佳的单层决策树
	Parameters:
		dataArr - 数据矩阵
		classLabels - 数据标签
		aggClassEst - 先前弱分类器的预测情况
		D - 样本权重
		V - 引入的样本权重
	Returns:
		bestStump - 最佳单层决策树信息
	...
	#################改进
	B_predict = np.mat(np.sign(aggClassEst))								#得到之前弱分类器的预测情况
	...
	for i in range(n):															#遍历所有特征
		...
		for j in range(-1, int(numSteps) + 1): 									
			for inequal in ['lt', 'gt']:  										#大于和小于的情况，均遍历。lt:less than，gt:greater than
				...
				##构造预测矩阵
				if B_predict.all()==0:			#因为第一轮还没有较强分类器和这些弱分类器去比较多样性，所以第一轮时,统一的指标
					DF=1
				else:
					A_predict = predictedVals.copy()
					# x=A_predict[B_predict==1]==1
					# # print(B_predict)
					# n11=np.size(x[x==True])
					# n10=np.size(x[x==False])
					x=A_predict[B_predict==-1]==-1
					n00=np.size(x[x==True])
					# n01=np.size(x[x==False])

					#双度误差，越小越好(实验证明这个效果不好)
					DF=n00/m
				...
				alpha=np.sign(alpha)*(abs(alpha)+C*(1-DF))						#加入多样性的考虑
				choice_proof=np.multiply(D,V).T * np.multiply((labelMat-alpha*predictedVals),(labelMat-alpha*predictedVals))
				###################

				# print('DF:{}'.format(DF))			

				if choice_proof < best_classifer: 						#在准确度的阈值下找到多样性最大的分类方式（关键是在于这个阈值如何确定）
				
					# print('有符合条件的了{}--{}--{}--{}'.format(choice_proof,P,best_classifer,weightedError))
...
def adaBoostTrainDS(dataArr, classLabels, numIt = 200):		
	...
	for i in range(numIt):
		...
		if i>0:	#第二次才奏效,为什么要第二次呢？因为第一次的aggClassEst不准确
			V,old_lam_1=AMSPL(i,classLabels,dataArr,aggClassEst,V,old_lam_1)
		####################
		bestStump, error, classEst = buildStump(dataArr,classLabels, D,V,aggClassEst) 	#构建单层决策树，返回弱学习器，他的错误率，以及预测值(离散，0/1)
