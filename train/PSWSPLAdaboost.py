...
def PSWSPL(numIt,classLabels,aggClassEst,v,lam=0.8,t=0.28):
	"""
	自适应混合权重的自步学习方法
		numIt 				- 当前自步学习的轮数
		classLabels 		- 样本标签值
...
def adaBoostTrainDS(dataArr, classLabels, numIt = 200):		
	...
	V =	np.mat(np.ones((m, 1))) 											#记录引入的样本权重（PSWSPL）
	...
	for i in range(numIt):
		...
		if i>0:	#第二次才奏效,为什么要第二次呢？因为第一次的aggClassEst不准确
			V=PSWSPL(i,classLabels,aggClassEst,V)
		####################
